...
export async function GET(_request: NextRequest, {params}: {params: Promise<{entityId: string}>}) {
    ...
    try {
        ...
        if (!detail) {
            return NextResponse.json({error: `Entity not found: ${entityId}`, code: 'NOT_FOUND'}, {status: 404});
        }
