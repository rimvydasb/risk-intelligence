...
describe('GET /api/v1/graph/expand/[jarKodas]', () => {
  ...
  it('returns 400 for invalid year param', async () => {
    const res = await GET(
      makeRequest('http://localhost/api/v1/graph/expand/110053842?year=notanumber'),
      { params: Promise.resolve({ jarKodas: '110053842' }) },
    );
    expect(res.status).toBe(400);
  ...
  it('passes filters to expandOrg', async () => {
    mockExpandOrg.mockResolvedValueOnce(MOCK_RESULT);
    await GET(
      makeRequest('http://localhost/api/v1/graph/expand/110053842?year=2022&minContractValue=5000'),
      { params: Promise.resolve({ jarKodas: '110053842' }) },
    );
