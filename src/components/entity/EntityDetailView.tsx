...
export function EntityDetailView({entityId}: EntityDetailViewProps) {
    ...
    const dataEntries = Object.entries(data.data ?? {}).filter(([k]) => k !== 'id' && k !== 'label' && k !== 'type');
                ...
                <CardContent sx={{display: 'flex', alignItems: 'flex-start', gap: 2, flexWrap: 'wrap'}}>
                    <Box sx={{flexGrow: 1}}>
                        ...
                        <Box sx={{display: 'flex', gap: 1}}>
                            <Chip label={data.type} color={typeColor} size="small" sx={{fontWeight: 600}} />
                            <Chip label={data.id} variant="outlined" size="small" sx={{fontFamily: 'monospace'}} />
                        </Box>
                    </Box>

                    {/* Risk Score */}
                    <Card variant="outlined" sx={{minWidth: 160, textAlign: 'center', background: '#1a1a2e'}}>
